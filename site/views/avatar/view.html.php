...
class P22eventosViewAvatar extends JView
{
	function display( $tpl = null )
	{
		...
		$uri = JFactory::getURI();
		$doc->addScript('./administrator/components/com_p22evento/js/jquery-pack.js');
		$doc->addScript('./administrator/components/com_p22evento/js/jquery.imgareaselect.min.js');
		...
		if( count( $post ) )
		{
			if( $post['act'] == 'upload' )
			{
				...
				{
					$image			= $model->lib->getDados( 'p22eventos_participantes' , 'p.avatar_img' , 'WHERE p.id = ' . intval( $post['pid'] ) , '' , 'loadResult' );
			...
			elseif( $post['act'] == 'upload_thumb' )
			{
				if( file_exists( JPATH_SITE . DS . $post['filepath'] ) )
				{
					...
					{
						if( $model->renomearRegistro( $post['pid'] , implode( DS , $thumbPathArray )  ) )
						{
							unlink( $filepath );
						}
					}
			...
			elseif( $post['act'] == 'all' )
			{
				...
				$model->renomearRegistro( $post['pid'] , $filepath  );
