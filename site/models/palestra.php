...
class P22eventosModelPalestra extends JModel
{
	...
	function &getRegistro()
	{
		...
		if ( empty( $this->_data ) )
		{
			...
			. ' ( SELECT COUNT(i.published) FROM #__p22eventos_inscritos AS i WHERE i.id_participante=p.id AND i.id_evento='. $this->idevento .' AND i.tp_reg=2 ) AS inscricao'
	...
	function &getRegistroPalestra()
	{
		$query = ' SELECT p.*'
		. ' FROM #__p22eventos_palestras AS p'
		. ' WHERE p.id=' . JRequest::getInt('palestraid');
		$this->_db->setQuery( $query );
		$palestra = $this->_db->loadObject();
	...
	private function _sendMail( &$user , $password )
	{
		...
		{
			$SEND_MSG	= "Olá %s,\n\nSeu cadastrado como palestrante no evento %s em %s foi realizado com sucesso.\n\nVocê agora pode efetuar o login em %s usando o Nome de Usuário e Senha:\n\nNome de Usuário - %s\nSenha - %s";
	...
	function delete()
	{
		...
		$aprovado	= $this->lib->hasData( $this->idevento , 'p22eventos_palestras' , 'aprovado' , $cids , 'id' , true );

		JArrayHelper::toInteger( $aprovado );
		...
		if (count( $cids ) )
		{
			foreach( $cids AS $cid )
			{
				if ( $aprovado[ $cid ] )
				{
					JError::raiseNotice( 100, 'Este palestra ou minicurso com o #ID ' . $cid . ' já foi aprovada. Você não pode mais excluí-la.' );
					$return = false;
					continue;
				}
				...
				if ( $grade[ $cid ] )
				{
					JError::raiseNotice( 100, 'Palestra ou Minicurso #ID ' . $cid . ' possui horários na grade vinculados a ela. Altere a grade de palestras e depois volte para excluí-la.' );
					$return = false;
					continue;
				}
