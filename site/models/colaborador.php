...
class P22eventosModelColaborador extends JModel
{
	...
	function &getRegistro()
	{
		...
		if ( empty( $this->_data ) )
		{
			...
			. ' ( SELECT COUNT(i.published) FROM #__p22eventos_inscritos AS i WHERE i.id_participante=p.id AND i.id_evento='. $this->idevento .' AND i.tp_reg=1 ) AS inscricao'
	...
	private function _sendMail( &$user , $password )
	{
		...
		{
			$SEND_MSG	= "Olá %s,\n\nSeu cadastrado como colaborador no evento %s em %s foi realizado com sucesso.\n\nVocê agora pode efetuar o login em %s usando o Nome de Usuário e Senha:\n\nNome de Usuário - %s\nSenha - %s";
