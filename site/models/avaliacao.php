...
class P22eventosModelAvaliacao extends JModel
{
	public $lib;
	public $idevento;
	...
	function __construct()
	{
		...
		if( !$this->id ) $mainframe->close('Palestra não informada');
	}
	...
	{
		$query = 'SELECT a.id_participante'
		. ' FROM #__p22eventos_avaliadores AS a'
		. ' WHERE a.id_evento=' . intval( $this->idevento )
		;
		$this->_avaliadores = $this->lib->getRegistrosCustom( $query , 'loadResultArray' );
	}
	...
	function getAvaliacoes()
	{
		...
		. ' p.nome AS palestra, p.aprovado AS status_palestra, p.id_trilha, p.nivel, t.nome AS trilha, p1.id AS id_participante,p.tipo,'
		...
		. ' WHERE a.id_evento=' . intval( $this->idevento )
		. ' AND p.id_evento=' . intval( $this->idevento ) . ' AND p.id=' . intval( $this->id )
		...
		for( $a = 0 ; $a < count( $result ) ; $a++ )
		{
			...
			$obj->tipo			= $row->tipo;
			$obj->id_palestra	= $row->id_palestra;
...
//		die;
	...
	public function getPontuacao()
	{
		...
		if( count( $this->_pontuacao[ $this->id ] ) )
		{
			$this->pontuacaoFinal[ $this->id ] = floor( array_sum( $this->_pontuacao[ $this->id ] ) / $num_avaliadores );
		}
