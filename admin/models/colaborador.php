...
class P22eventosModelColaborador extends JModel
{
	public $id;
	public $lib;
	public $idevento;
	...
	function &getRegistro()
	{
		...
		if ( empty( $this->_data ) )
		{
			...
			. ' WHERE i.tp_reg=1 AND i.id_evento='. intval( $this->idevento ) .' AND i.id_participante = ' . intval( $this->id );
	...
	function delete()
	{
		...
		$avaliador = $this->lib->isAvaliador( $this->idevento , $cids );
		...
		foreach( $cids AS $cid )
		{
			if ( $avaliador[ $cid ] )
			{
				JError::raiseNotice( 100, 'O Colaborador #ID ' . $cid . ' está cadastrado como Avaliador. Remova-o como Avaliador e tente excluí-lo novamente.' );
				$return = false;
				continue;
			}
