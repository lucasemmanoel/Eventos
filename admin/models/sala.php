...
class P22eventosModelSala extends JModel
{
	public $id;
	public $idevento;
	...
	function &getRegistro()
	{
		...
		if ( empty( $this->_data ) )
		{
			$query = ' SELECT * FROM #__p22eventos_salas'.
	...
	function delete()
	{
		...
		$palestra = $this->lib->hasData( $this->idevento , 'p22eventos_grades' , 'id_sala' , $cids );
		...
		if (count( $cids ) )
		{
			foreach( $cids AS $cid )
			{
				if ( $palestra[ $cid ] )
				{
					JError::raiseNotice( 100, 'A Sala #ID ' . $cid . ' possui palestras ou mini-cursos vinculados a ela. Altere a(s) palestra(s)/Mini-curso(s) vinculados a esta sala e depois volte para excluí-la.' );
					$return = false;
					continue;
				}
