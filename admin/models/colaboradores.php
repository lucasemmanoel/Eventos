...
class P22eventosModelColaboradores extends JModel
{
	...
	{		
		...
		if ( empty( $this->_registros ) )
		{
			...
			$query = 'SELECT i.registrado_em, i.published, p.id, u.name'
			...
			. ' WHERE i.tp_reg=1 AND u.id = p.id_user '
			. $where
			. $orderby
			;
	...
	public function getProfissoes()
	{
		...
		$prof = $this->lib->getDados( 'p22eventos_profissoes' , 'p.id , p.nome' , ' WHERE p.id IN (' . implode( ',' , $reg_prof ) . ')' , 'ORDER BY p.nome' , 'loadObjectList' );
