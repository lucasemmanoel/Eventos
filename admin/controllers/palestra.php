...
class P22eventosControllerPalestra extends P22eventosController
{
	...
	function save()
	{
		...
		{
			case 'apply':
				$link = 'index.php?option=com_p22evento&controller=palestra&task=edit&idevento=' . intval( $data['idevento'] ) . '&cid[]=' . intval( $model->id );
				$this->setRedirect( $link, $msg , $msgType );
				break;

			case 'save':
			default:
				$link = 'index.php?option=com_p22evento&task=palestras&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
			case 'save_continue':
				$link = 'index.php?option=com_p22evento&controller=palestra&task=add&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
		}
	}
	...
	function remove()
	{
		...
		$link = 'index.php?option=com_p22evento&task=palestras&idevento=' . intval( $idevento );
		$this->setRedirect( $link, $msg , $msgType );
	}
