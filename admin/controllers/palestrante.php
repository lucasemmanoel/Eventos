...
class P22eventosControllerPalestrante extends P22eventosController
{
	/*
	...
	function edit()
	{
		JRequest::setVar( 'view', 'palestrante' );
	...
	function save()
	{
		...
		$model				= $this->getModel( 'palestrante' );
		...
		{
			case 'apply':
				$link = 'index.php?option=com_p22evento&controller=palestrante&task=edit&idevento=' . intval( $data['idevento'] ) . '&cid[]=' . intval( $model->id );
				$this->setRedirect( $link, $msg , $msgType );
				break;

			case 'save':
			default:
				$link = 'index.php?option=com_p22evento&task=palestrantes&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
			case 'save_continue':
				$link = 'index.php?option=com_p22evento&controller=palestrante&task=add&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
		}
	}
	...
	function remove()
	{
		...
		$link = 'index.php?option=com_p22evento&task=palestrantes&idevento=' . intval( $idevento );
		$this->setRedirect( $link, $msg , $msgType );
	}
	...
	function publish( )
	{
		...
		. ' WHERE id_participante IN ( '.$cids.' ) AND tp_reg=2'
		;
		$db->setQuery( $query );
