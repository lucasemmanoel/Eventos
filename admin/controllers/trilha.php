...
class P22eventosControllerTrilha extends P22eventosController
{
	/*
	...
	function save()
	{
		...
		{
			case 'apply':
				$link = 'index3.php?option=com_p22evento&controller=trilha&task=edit&idevento=' . intval( $data['idevento'] ) . '&cid[]=' . intval( $model->id );
				$this->setRedirect( $link, $msg , $msgType );
				break;

			case 'save':
			default:
				$link = 'index3.php?option=com_p22evento&task=trilhas&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
			case 'save_continue':
				$link = 'index3.php?option=com_p22evento&controller=trilha&task=add&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
		}
	}
	...
	function remove()
	{
		...
		$link = 'index3.php?option=com_p22evento&task=trilhas&idevento=' . intval( $idevento );
		$this->setRedirect( $link, $msg , $msgType );
	}
	...
	function publish( )
	{
		...
		$query = 'UPDATE #__p22eventos_trilhas SET published = '.(int) $publish
		. ' WHERE id IN ( '.$cids.' )'
		;
		$db->setQuery( $query );
