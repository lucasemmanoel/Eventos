...
class P22eventosControllerMinicurso extends P22eventosController
{
	/*
	...
	function edit()
	{
		JRequest::setVar( 'view', 'minicurso' );
	...
	function save()
	{
		...
		{
			case 'apply':
				$link = 'index.php?option=com_p22evento&controller=minicurso&task=edit&idevento=' . intval( $data['idevento'] ) . '&cid[]=' . intval( $model->id );
				$this->setRedirect( $link, $msg , $msgType );
				break;

			case 'save':
			default:
				$link = 'index.php?option=com_p22evento&task=minicursos&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
			case 'save_continue':
				$link = 'index.php?option=com_p22evento&controller=minicurso&task=add&idevento=' . intval( $data['idevento'] );
				$this->setRedirect( $link, $msg , $msgType );
				break;
		}
	}
	...
	function remove()
	{
		...
		$link = 'index.php?option=com_p22evento&task=minicursos&idevento=' . intval( $idevento );
		$this->setRedirect( $link, $msg , $msgType );
	}
	...
	function publish( )
	{
		...
		$query = 'UPDATE #__p22eventos_palestras SET published = '.(int) $publish
		. ' WHERE id IN ( '.$cids.' )'
		;
		$db->setQuery( $query );
