...
class P22eventosViewPalestrante extends JView
{
	function display( $tpl = null )
	{
		...
		JToolBarHelper::title(   JText::_( 'Palestrante' ).': <small><small>[ ' . $text.' ]</small></small>', 'p22Palestrantes'  );
		...
		include( JPATH_COMPONENT . DS . 'models' . DS . 'avatar.php' );

		// Conecta-se com o model.
		$model				=& $this->getModel();
		$eventName			= $model->lib->eventName();
		$dirEventName	= JFolder::makeSafe( P22eventosModelAvatar::remove_acentos( $eventName ) );
		...
		$profissoes				= $model->lib->getProfissoes();
		...
		$this->assignRef( 'dirEventName', $dirEventName );
