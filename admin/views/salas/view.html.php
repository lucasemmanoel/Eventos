...
class P22eventosViewSalas extends JView
{
	function display( $tpl = null )
	{
		...
		$idlocal	= $this->get( 'Local' );
		$nome		= 'Sala';
		$eventName	= $this->get('EventName');
		$titulo		= JText::_( 'Evento' ) . ': <small style="color: #333">' . $eventName . '</small>';

		JToolBarHelper::title( JText::_( $titulo ) , 'p22Salas' );
		
		JToolBarHelper::publishList();
		JToolBarHelper::unpublishList();
		JToolBarHelper::deleteList( 'Tem certeza que deseja excluir esta '. $nome .'?' );
		...
		$this->assignRef( 'idlocal', $idlocal );
