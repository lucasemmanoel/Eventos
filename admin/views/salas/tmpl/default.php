...
<form action="index.php?option=com_p22evento&task=salas" method="post" name="adminForm">
	<div id="editcell">
		<table>
			<tr>
				<td colspan="2">
		...
		<table class="adminlist">
			<thead>
				<tr style="background-color: #f2f2f2;">
					...
					<th nowrap="nowrap" class="title" style="text-align:left">
						<?php echo JHTML::_('grid.sort',  'Nome', 'p.nome', @$this->lists[ 'order_Dir' ], @$this->lists[ 'order' ] , 'salas' ); ?>
					</th>
			...
			<tbody>
				...
				for ($i = 0, $n=count( $this->registros ); $i < $n; $i++)
				{
					...
					$link		= JRoute::_( 'index.php?option=com_p22evento&controller=sala&idevento='. intval( $this->idevento ) .'&idlocal='. intval( $this->idlocal ) .'&task=edit&cid[]='. $row->id );
					?>
	...
	<input type="hidden" name="task" value="salas" />
	<input type="hidden" name="idevento" value="<?php echo $this->idevento; ?>" />
	<input type="hidden" name="idlocal" value="<?php echo $this->idlocal; ?>" />
